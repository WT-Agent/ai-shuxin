  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">收信人关系</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">书信主题</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">真情书信撰写与沟通破冰指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 情感书信与家书寄怀专家');
...
const metricsList = [
  {
    "key": "emotionalSincerityDepth",
    "label": "情感真挚感染力"
  },
  {
    "key": "epistolaryElegance",
    "label": "书信体裁优雅度"
  },
  {
    "key": "empathyResonanceIndex",
    "label": "共情共鸣指数"
  },
  {
    "key": "rhetoricalWarmthGrade",
    "label": "修辞温度评分"
  },
  {
    "key": "relationshipHealingImpact",
    "label": "关系修复促进分"
  }
];
const guideRules = [
  {
    "title": "细节描写胜过空洞表白",
    "advice": "回忆具体生活细节（如“记忆中母亲熬的红豆汤”），用场景引发对方的情感共鸣",
    "avoid": "切忌大段堆砌“我很爱你”、“我非常抱歉”等空洞口号"
  },
  {
    "title": "真诚坦白内心脆弱",
    "advice": "在道歉信或沟通信中，坦诚承认自己的不完美与当时的情绪，表现出换位思考",
    "avoid": "避免在信中继续辩解或归咎于对方"
  },
  {
    "title": "书信仪式感留存",
    "advice": "建议使用精美信纸手写，字迹工整，手写信比电子文字多一份沉甸甸的情感温度",
    "avoid": "严禁直接复制粘帖过于冷冰冰的 AI 通用套话"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[SHUXIN_SCORES\][\s\S]*?\[\/SHUXIN_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[SHUXIN_SCORES\](.*?)\[\/SHUXIN_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('shuxin_history_records');
...
const saveHistory = () => {
  localStorage.setItem('shuxin_history_records', JSON.stringify(historyList.value));
};
